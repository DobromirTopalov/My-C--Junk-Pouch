...
int main()
{
	...
	int size;
	const int sizeHighCap = pow(10, 9);
	const int sizeLowCap = pow(10, 6);

	std::cin >> size;							//get size of the sequence

	if (size > sizeHighCap) {					//check if input is correct
		std::cout << "no\n";					//print message 
		return 1;								//error
	}
	...
	if (size > 2) {
	 ...
	 //{	without using dynamic arrays we have to get input and work on it directly,so we do it over and over ...
								//first two inputs must be by the user after this we just swap values 	
	
		std::cin >> firstElement >> secondElement;
		if ( abs(firstElement) > sizeLowCap || abs(secondElement) > sizeLowCap) {			//check if input is okay
			std::cout << "no\n";					//print message 
			return 1;								//error
		}
		...
		for (int i = 0; i < size - 2; i++) {
			...
			if ( abs(thirdElement) > sizeLowCap) {			//check if input is okay
