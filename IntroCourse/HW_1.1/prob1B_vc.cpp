...
int main()
{
	...
	while (number)												//to loop the bits
	{
		...
		if (countTheSequence >= 2)								//a sequence is 2 or more same bits
		{
			...
			if (bit == 1)															//if the first bit is 1
			{
				...
				{																	//Examples: 3/2 = 1 | 6/2 = 3 | 7/2 = 3 | 8/2 = 4	
					...
					if (countTheSequence % 2 == 0 && countTheSequence >= 2)			//move mask one more time if we have a sequence (2,4,6..)
						beginningOfBitChange <<= 1;
					if (countTheSequence % 2 != 0 && countTheSequence > 2)			//. . .									seq (3,5,7...)
						beginningOfBitChange <<= 1;									//. . .
		...
		{
			beginningOfBitChange <<= 1;				//move mask if no seq
		}
