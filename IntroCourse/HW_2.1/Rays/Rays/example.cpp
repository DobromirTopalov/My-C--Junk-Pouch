...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	do {
		...
		unsigned int maxInputSize = 11 + 4 * maxMirrorsInRoom - 1;
				...
				/*
		std::cout << input[19] << " " << input[20] << " " << input[21] << " " << input[22] << "\n";
		point = findPointOfIntersection(&point, beginningPoint_X, beginningPoint_Y, direction_X, direction_Y, 75, 50, 699, 388);
		if (isPointOnSegment(point, 75, 50, 699, 388))
		{
			SetRaySegment(beginningPoint_X, beginningPoint_Y, point[0], point[1]);
		}
		nextPoint = reflectedPoint(&nextPoint, point[0], point[1], 75, 50, 699, 388, beginningPoint_X, beginningPoint_Y);
		SetRaySegment(point[0], point[1], nextPoint[0], nextPoint[1]);


		double oldX = point[0];
		double oldY = point[1];
		point = findPointOfIntersection(&point, point[0], point[1], nextPoint[0],nextPoint[1], wallCoordinates[8], wallCoordinates[9], wallCoordinates[10], wallCoordinates[11]);
		
		SetRaySegment(point[0], point[1],target_X,target_Y);
		
		std::cout << isPointOnSegment(point, wallCoordinates[8], wallCoordinates[9], wallCoordinates[10], wallCoordinates[11]);
		if (isPointOnSegment(point, wallCoordinates[8], wallCoordinates[9], wallCoordinates[10], wallCoordinates[11]))
		{
			SetRaySegment(oldX, oldY, point[0],point[1]);
			std::cout << "dsa";
		}
				*/
		...
		/*
		SetRaySegment(1, 1, input[0], 1);
		point = findPointOfIntersection(&point, beginningPoint_X, beginningPoint_Y, direction_X, direction_Y, 1, 1, input[0], 1);
		SetRaySegment(beginningPoint_X, beginningPoint_Y, point[0], point[1]);
		nextPoint = reflectedPoint(&nextPoint, point[0], point[1], 1, 1, input[0], 1, beginningPoint_X, beginningPoint_Y);
		SetRaySegment(point[0], point[1], nextPoint[0], nextPoint[1]);
		*/
...
double* reflectedPoint(double **buffer, const double& pointOnObject_X, const double& pointOnObject_Y, const double& object_X1, const double& object_Y1, const double& object_X2, const double& object_Y2, const double& pointToReflect_X, const double& pointToReflect_Y)
{
	...
	if (!(1 + M2*M2))								//check for deletion by 0 before init D
	{
		std::cout << "Fail_3\n";
		return *buffer;
	}
	double D = (pointToReflect_X + (pointOnObject_Y - B)*M2) / (1 + M2*M2);			//needed for a formula

	double X = 2 * D - pointToReflect_X;					// x coordinate of the point
	double Y = 2 * D * M2 - pointOnObject_Y + 2 * B;		// y coordinate of the point
...
//	THIS IS A COPY or First version of the file //
//				 BACKUP if fail					//
...
//
// Тук напишете своето решение, като не забравяте да извиквате
// SetRaySegment() за всеки сегмент от лъча, който изчислите.
//
// По-долу е даден пример с генериране на произволни сегменти,
// всеки от които започва там, където свършва предишният.
// Не се отчита какви огледала има в стаята и без да
// се смятат точки на отражение.
//
...
unsigned int maxReflections = (unsigned int)input[9];
unsigned int maxMirrorsInRoom = (unsigned int)input[10];
unsigned int maxInputSize = 11 + 4 * maxMirrorsInRoom - 1;


for (int c = 0;  c < 6; c++) {
SetRaySegment(beginningPoint_X, beginningPoint_Y, direction_X, direction_Y);
Sleep(100);

beginningPoint_X = direction_X;
beginningPoint_Y = direction_Y;
direction_X = rand() % (int)input[0];
direction_Y = rand() % (int)input[1];
}
...
for (int i = 12; i < maxInputSize; i += 4)
{
point = findPointOfIntersection(&point, beginningPoint_X, beginningPoint_Y, direction_X, direction_Y, input[i], input[i + 1], input[i + 2], input[i + 3]);
bool isOnMirror = isPointOnSegment(point, input[i], input[i + 1], input[i + 2], input[i + 3]);
...
SetRaySegment(beginningPoint_X, beginningPoint_Y, closestPoint[0], closestPoint[1]);
nextPoint = reflectedPoint(&nextPoint, closestPoint[0], closestPoint[1],input[indexOfCloseObject],input[indexOfCloseObject+1],input[indexOfCloseObject+2],input[indexOfCloseObject+3],beginningPoint_X,beginningPoint_Y);
SetRaySegment(closestPoint[0], closestPoint[1], nextPoint[0], nextPoint[1]);
maxReflections--;
...
else if (beginningPoint_X < direction_X)//right
{
range_X = beginningPoint_X;
...
else if (beginningPoint_X == direction_X && beginningPoint_Y > direction_Y)//top
{
range_Y = beginningPoint_Y;
...
if (currentClosestPoint[1] > closestPoint[1])
{
indexOfCloseObject = i;
closestPoint[0] = currentClosestPoint[0];
closestPoint[1] = currentClosestPoint[1];
...
if (isAnyMirrorHit)
{
if (isTargetBOOM && (pointOnTheTarget[1] > closestPoint[1]))
{
SetRaySegment(beginningPoint_X, beginningPoint_Y, pointOnTheTarget[0], pointOnTheTarget[1]);
std::cout << "Congratulations!You've hit the target!\n";
return 0;
...
else if (beginningPoint_X == direction_X && beginningPoint_Y < direction_Y)//bottom
{
range_Y = beginningPoint_Y;
